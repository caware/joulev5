...
// Public methods
	...
	//Getters and Setters
	this.setMetricType = function(_metricType){metricType = _metricType};
	this.getMetricType = function(){return metricType};
	
	this.setPlotData = function(_plotData){plotData = _plotData};
	this.getPlotData = function(){return plotData};
	
	this.setViewRange = function(_viewRange){viewRange = _viewRange};
	this.getViewRange = function(){return viewRange};
	...
	this.changeRange = function (range) {
		...
		viewRange.start = range[0];
		viewRange.end = range[1];
		updateSuffixes();
		for (var i=0; i< cvis.tree.plotLineDesc.length; i++) cvis.tree.plotLineDesc[i].valid = false;
...
// Private methods
	...
	var getSuffixes = function (interval) {
		...
		var runningDate = new Date(viewRange.start);
		var monthStr;
		var yearStr; 
		...
		switch (interval) {
			case "monthly": // Earlier year, or same year and earlier or same month
				while ((runningDate.getFullYear() < viewRange.end.getFullYear()) ||
					((runningDate.getFullYear() == viewRange.end.getFullYear()) &&
					(runningDate.getMonth() <= viewRange.end.getMonth()))) {
			...
			case "yearly":
				...
				break;
			default:
				while (runningDate.getMonth() <= viewRange.end.getMonth()) {
					yearStr = runningDate.getFullYear().toString();
					monthStr = zero(runningDate.getMonth()+1);
					suffixes.push({"year" : yearStr, "month" : monthStr});
					runningDate.addMonths(1);
	...
	var createTable = function () {
		...
		for (var i=0; i < cvis.tree.selected.length; i++) {
			...
			if (metricType == "carbon") { // Create data for the table
				table.push({
					"colour": pL.id,
		...
		function validateData (pL) { // Provide plot lines with final readings
			...
			if (pL.hasOwnProperty("primary")){ 
				...
				if (pL.hasOwnProperty("secondary")){
					pL.data = cvis.utils.altDataReadings (pL.data, getPlotLineData(pL.secondary[0]));
				}
				return;
			}
		}
	...
	// Function code
		...
		for (var p=0; p<cvis.tree.plotLineDesc.length; p++) { // For each plot line in plotLineDesc
			...
			if (!pL.processed) { // Process data to set start and end dates and fix bad data
				...
				pL.points.data = _.filter(pL.points.data, function (v) { return v.x.between(viewRange.start, viewRange.end) });
				...
				if (points > 0) {
					...
					avg = cvis.utils.avg(_.filter(pL.points.data, function (v) { return v.x.between(viewRange.start, viewRange.end) }));
	...
	var sankeyTree = function (rootId, maxDepth, step, interval) {
		...
		if (typeof rootId == 'undefined') {
			if (typeof plotData.rootId != 'undefined')
				rootId = plotData.rootId;
			else 
				rootId = 0;
		}
		...
		return update().then(function () {
			...
			if (typeof plotData.interval == 'undefined')
				interval = 'hourly';
			else
				interval = plotData.interval;
	...
	var update = function () {
		...
		return $.when(
			...
			while(validSelection === false && iterations < 5) {
				...
				that.changeRange([viewRange.end.clone().addMonths(-1).clearTime().moveToFirstDayOfMonth(), viewRange.end.clearTime().addMonths(-1).moveToLastDayOfMonth()]);
				that.resetSelection();
				loadMeterData(getMeterNames());
				loadCarbonData();
				validSelection = createPoints();
				iterations++;
				console.log(iterations, validSelection, viewRange);
			}
		});
