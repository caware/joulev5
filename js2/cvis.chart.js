...
function chart(options) {
	...
	var filter = false;
	var areas = typeof options.areas == 'boolean' ? options.areas : true;
	var scaleSelection = "scale";
	var cols = utils.nulls(10);
	var type = options.type || "sankey";
	var changing = true;
	var data = {};
	var rootData = {};
	var dim = {
...
// Public methods
	...
	//Setters & Getters
	this.setFilter = function(_filter){filter = _filter};
	this.getFilter = function(){return filter};
	
	this.setAreas = function(_areas){areas = _areas};
	this.getAreas = function(){return areas};
	
	this.setScaleSelection = function(_scaleSelection){scaleSelection = _scaleSelection};
	this.getScaleSelection = function(){return scaleSelection};
	
	this.setCols = function(_cols){cols = _cols};
	this.getCols = function(){return cols};
	
	this.setType = function(_type){type = _type};
	this.getType = function(){return type};
	
	this.setChanging = function(_changing){changing = _changing};
	this.getChanging = function(){return changing};
	
	this.setData = function(_data){data = _data};
	this.getData = function(){return data};
	
	this.setRootData = function(_rootData){rootData = _rootData};
	this.getRootData = function(){return rootData};
	
	this.setDim = function(_dim){dim = _dim};
	...
	this.redraw = function (options) { // Refresh a chart
		...
		cvis.data.createPlotData(type, options).then(function() {
	...
	this.tree = function (sensorTree) { // Initialise the tree
		...
		rootData.name = cvis.tree.title;
		rootData.x0 = 0;
		rootData.y0 = 0;
		rootData.treeDim = treeDim;
		rootData.children = cvis.utils.mapper(cvis.tree.plotLineDesc);
	...
	this.treeUpdate = function (options) { // Update the tree
		...
		cvis.utils.updater(rootData, cvis.tree.plotLineDesc, options.expand); // Update the tree data depending on the selected plot lines

		var treeDim = rootData.treeDim;
		var nodes = that.tree.nodes(rootData);
...
// Private methods
	...
	var sliceData = function (range) { // Slices the data so that the focus only shows the selected range. It also filters data by sampling and applying a gaussian filter
		...
		if (typeof range == 'undefined') {
			...
			chartTemp.focusData = cvis.data.getPlotData().data;
			chartTemp.fx.domain([cvis.data.getPlotData().start, cvis.data.getPlotData().end]);
		} else {
			// Set the X domain to range
			chartTemp.fx.domain(range);
			// Slice the data array to include data within range
			var slicedData = [];
			for(var j=0; j<cvis.data.getPlotData().data.length; j++){
				slicedData[j] = [];
				var remapped = _.map(_.pluck(cvis.data.getPlotData().data[j].data, "x"), function (d) { return d.getTime(); }); // Map elements by x property and convert to timestamp
				slicedData[j].data = (cvis.data.getPlotData().data[j].data.slice(
				...
				filter ? slicedData[j].data = gaussian(5, 3, slicedData[j].data) : null;
				slicedData[j].id = cvis.data.getPlotData().data[j].id;
				slicedData[j].avg = cvis.data.getPlotData().data[j].avg;
			}
		...
		var trimmedData = [];
		if (cvis.data.getPlotData().data.length > 0) {
			// Sample the data for the context if more than 600 points are being displayed
			if (cvis.data.getPlotData().data[0].data.length < 600) {
				for (var i=0; i < cvis.data.getPlotData().data.length; i++) {
					trimmedData[i] = new Object();
					trimmedData[i].id = cvis.data.getPlotData().data[i].id;
					trimmedData[i].data = [];
					filter ? trimmedData[i].data = gaussian(5, 3, cvis.data.getPlotData().data[i].data) : trimmedData[i].data = cvis.data.getPlotData().data[i].data;
				}
			} else {
				var mydiv = cvis.data.getPlotData().data[0].data.length / 600;
				var rounddiv = Math.round(mydiv*1)/1;
				
				for(var p=0; p<cvis.data.getPlotData().data.length; p++){
					for(var q=0; q<cvis.data.getPlotData().data[p].data.length;q++){
						if (q == 0) {
							trimmedData[p] = {};
							trimmedData[p].data = [];
						}
						if (q%rounddiv == 0) {
							trimmedData[p].data.push(cvis.data.getPlotData().data[p].data[q]);
						}
					}
					filter ? trimmedData[p].data = gaussian(5, 3, trimmedData[p].data) : null;
					trimmedData[p].id = cvis.data.getPlotData().data[p].id;
					trimmedData[p].avg = cvis.data.getPlotData().data[p].avg;
				}
			}
		}
	...
	var line = function () { // Initiliase a line chart
		...
		chartTemp.color = function (id) {
			...
			_.each(cols, function (v, k, l) { _.some(cvis.tree.selected, function (d) { return d.id == v; }) ? l[k] = v : l[k] = null; });

			var index =_.indexOf(cols, id) == -1 ? _.indexOf(cols, null) :  _.indexOf(cols, id);
	...
	var lineUpdate = function (options) { // Update the line chart
		...
		if (!options.refresh) {
			...
			chartTemp.cx.domain([cvis.data.getPlotData().start, cvis.data.getPlotData().end]);
			chartTemp.cy.domain([0, cvis.data.getPlotData().yMax]);
			// Check the brush and set its extent to either the intial values or to the same values when the range changes
			if (chartTemp.cBrush.empty() || chartTemp.cBrush.extent()[0].isBefore(cvis.data.getPlotData().start)) {
			...
			switch (scaleSelection) {
				case 'all':
					chartTemp.fy.domain([0, cvis.data.getPlotData().yMax * 1.1]);
					break;
				case 'scale':
			...
			chartTemp.focus.selectAll(".f-area")
				...
				.append("svg:path")
					...
					.style("fill", function(d, i) { // Create the gradient for the color used by the chart if it doesn't exist
							if (chartTemp.container.select("#grad" + _.indexOf(cols, d.id)).empty()) {
								var gradient = chartTemp.defs.append("linearGradient")
										.attr("id", "grad" + _.indexOf(cols, d.id))
										.attr("x1", "100%")
										.attr("x2", "100%")
										.attr("y1", "100%")
										.attr("y2", "0%");
						...
						return "url(#grad" + _.indexOf(cols, d.id) + ")";
					})
					.style("display", areas ? "inline" : "none");
			// Sort areas as above
		...
		_.each(cvis.data.getPlotData().data, function(d, i) {
	...
	var gridUpdate = function () { // Update or create the data table
		...
		if (cvis.data.getMetricType() == "carbon") {
			_.each(chartTemp.focusData.data, function (d, i) {i < cvis.data.getPlotData().table.length ? cvis.data.getPlotData().table[i].avgselected = carbonFormatter(d3.mean(d.data, function (e) { return (typeof e.y === "number") ? e.y : 0; })) : null; });
			columns = [
			...
			_.each(chartTemp.focusData.data, function (d, i) {i < cvis.data.getPlotData().table.length ? cvis.data.getPlotData().table[i].avgselected = pwrFormatter(d3.mean(d.data, function (e) { return (typeof e.y === "number") ? e.y : 0; })) : null; });
			columns = [
			{id:"PlotColour", name:"Colour", field:"colour", width:60, formatter: Slick.Formatters.Color(chartTemp.color)},
			{id:"Description", name:"Description", field:"description", width:195},
			{id:"StartMonthyear", name:"Start", field:"startmonthyear", width:80},
		...
		if (typeof that.grid == 'undefined') {
			that.grid = new Slick.Grid("#data-table", cvis.data.getPlotData().table, columns, options);
		} else {
		that.grid.setData(cvis.data.getPlotData().table);
	...
	var sankeyUpdate = function (options) { // Update the sankey diagram
		...
		that.treeUpdate({}); // Update the tree
		var sData = cvis.data.getPlotData();
	...
	var treeMapUpdate = function (options) { // Update the tree map
		...
		chartTemp.y = d3.scale.linear()
			...
			.range([0, dim.h1]);
		that.treeUpdate({}); // Update the tree
		var root = cvis.data.getPlotData();
